...
function Delete(id) {
	...
	if (cf == true) {
		$.ajax({
			url: apiFetch = `http://localhost:8081/websiteVali/api/dong-san-pham/xoa?id=${ id }`,
			type: 'DELETE',
			success: function (result) {
				if(result == 1){
					window.location.href = 'http://localhost:8081/websiteVali/admin/dong-san-pham/danh-sach-dong-san-pham';
