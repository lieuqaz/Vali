...
$("#btnGuiDanhGia").click(function (e) {
	...
	$.ajax({
		url : "http://localhost:8081/websiteVali/api/hoa-don/danh-gia",
		type : "post",
		data : JSON.stringify({
...
muaLai = (id) => {
	window.location.href = "http://localhost:8081/websiteVali/san-pham?id=" + id;
}
huyDonHang = (id) => {
	var r = confirm("Bạn có muốn hủy đơn hàng?");
	if(r == true){
		window.location.href = "http://localhost:8081/websiteVali/quan-ly-don-hang/huy-don-hang?id=" + id;
	}
}
