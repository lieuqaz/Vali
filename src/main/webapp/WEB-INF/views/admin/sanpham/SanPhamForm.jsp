...
<body>
	...
	<button
		onClick="window.location.href='http://localhost:8081/websiteVali/admin/san-pham/danh-sach-san-pham'"
		class="btn btn-success">Quay lại</button>
				...
				<form:form action="" method="POST" modelAttribute="sanPhamDTO"
					...
					<div class="row">
						<fmt:formatNumber type="number" groupingUsed="false"
							value="${ sanPhamDTO.giaNhap }" var="giaNhap" />
						<div class="col-lg-6">
							<div class="form-group">
								<label>Giá nhập:</label>
								<form:input path="giaNhap" value="${giaNhap}"
									class="form-control" type="number" />
								<form:errors path="giaNhap" cssClass="error"></form:errors>
							</div>
						</div>
						<fmt:formatNumber type="number" groupingUsed="false"
							value="${ sanPhamDTO.giaBan }" var="giaBan" />
					...
					<div class="row">
								...
								<label>Loại sản phẩm:</label> <select id="maLoaiSanPham"
									...
									<c:forEach items="${ loaiSanPhams }" var="loaiSanPham">
										<option value="${loaiSanPham.id}"
											<c:if test="${loaiSanPham.id == sanPhamDTO.maLoaiSanPham}">selected</c:if>>${loaiSanPham.tenLoaiSanPham}</option>
									</c:forEach>
								...
								<label>Thương hiệu:</label> <select id="maThuongHieu"
									...
									<c:forEach items="${ thuongHieus }" var="thuongHieu">
										...
										<option value="${thuongHieu.id} "
											<c:if test="${thuongHieu.id == sanPhamDTO.maThuongHieu}">selected</c:if>>${thuongHieu.tenThuongHieu}</option>
						...
						<div class="col-lg-6">
							<div class="form-group">
								<label>Dòng sản phẩm:</label> <select name="maDongSanPham"
									id="maDongSanPham" Class="form-control">
									<c:forEach items="${ dongSanPhams }" var="dongSanPham">
										<option value="${dongSanPham.id} "
											<c:if test="${dongSanPham.id == sanPhamDTO.maDongSanPham}">selected</c:if>>${dongSanPham.tenDongSanPham}</option>
									</c:forEach>
								</select>
							</div>
						</div>
					...
					<div class="form-group">
						<label>Mô tả:</label>
						<div id="editor">
							<div id="edit" style="margin-top: 10px"></div>
						</div>
						<form:hidden path="moTa" value="${sanPhamDTO.moTa }" />
					</div>


					<button type="submit" class="btn btn-success">${ formButton }</button>
					<button type="button" class="btn btn-danger"
						onClick="window.location.href='http://localhost:8081/websiteVali/admin/san-pham/danh-sach-san-pham'">Hủy</button>
